 ...
 * the code; the one action is the filled button after a rule — not chips and
 * a button in the same face (*"ciężko rozróżnić, co jest opcją, a co
 * akcją"*). ✕ puts it away until the findings change.
 *
...
const DeclareBar = ({ missing, onInsert }) => {
  ...
  return (
    <div className="flex max-w-full flex-wrap items-center gap-x-3 gap-y-1.5 rounded-ctl border border-amb bg-panel px-3 py-2 text-note">
      ...
      <button
        ...
        className="text-base leading-none text-mut hover:text-ink"
      >
        ✕
      </button>
    </div>
