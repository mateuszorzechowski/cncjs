...
describe('emergencyStop on Grbl', () => {
  beforeEach(() => {
    jest.useFakeTimers();
...
describe('emergencyStop on a controller whose server side cannot do it', () => {
  beforeEach(() => {
    jest.useFakeTimers();
    controller.command.mockClear();
  });
   ...
   *
   * `onStop={emergencyStop}` hands the click event in as the first argument, so
